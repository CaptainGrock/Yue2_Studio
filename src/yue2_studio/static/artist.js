...
function bindArtistTrainer(){
  ...
  $('artistTrain').onclick=()=>{
    ...
    const payload={project_id,gpu_confirmed:true,steps:Number($('artistSteps').value),rank:Number($('artistRank').value),learning_rate:Number($('artistLearningRate').value),lr_schedule:$('artistLrSchedule').value,checkpoint_every:Number($('artistCheckpoint').value),alignment_weight:Number($('artistAlignment').value),alignment_method:$('artistAlignmentMethod').value};
    confirmReplace('Queue Artist LoRA training?','Uses the selected saved setup, not unsaved edits. Lyrics are cleaned automatically into the frozen job copy for both model conditioning and alignment; source sidecars are not overwritten, and the Library run keeps cleaned copies plus a change log. '+(payload.alignment_weight===0?'Lyric timing is skipped.':payload.alignment_method==='whisper'?'Whisper-assisted lyric timing will save a per-song review JSON.':'MMS lyric alignment will run.')+' Whole-song encoding runs before '+payload.steps+' training steps in the shared GPU queue. Playback requires No score, Torch, quantization None, and AR offloading disabled.',()=>busy('artistTrain',async()=>{
