...
def prepare(spec,directory,tokenizer,device):
    ...
    for row in rows:
        ...
        identity=dict(version='artist-whole-song-v1',audio=audio_hash,lyrics=row['lyrics_sha256'],mert=identities,encoder=REVISION,
                      alignment='htdemucs-shifts0-mms-fa-english-v1' if aligned else 'none')
        key=hashlib.sha256(json.dumps(identity,sort_keys=True).encode()).hexdigest()
        folder=cache/key;folder.mkdir(exist_ok=True)
        style=project['trigger']+', '+project['shared_style']
        item=dict(name=row['name'],src='artist',style=style,lyrics=row['lyrics'],source=source,folder=folder,identity=identity)
    ...
    if aligned and missing:
        ...
        print('Loading htdemucs vocal separator (first use downloads its official weights).',flush=True)
        demucs=get_model('htdemucs').eval()
