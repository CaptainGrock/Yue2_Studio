...
def _exclusive(function):
    """Serialize model use; reject changes from another thread or callbacks."""
    @wraps(function)
    def guarded(self, *args, **kwargs):
        lock = self.__dict__.setdefault("_operation_lock", threading.RLock())
        if not lock.acquire(blocking=False):
            raise RuntimeError("Pipeline is busy; wait until generation finishes")
        self._operation_depth = getattr(self, "_operation_depth", 0) + 1
        try:
...
class YuE2Pipeline:
    ...
    def load_lora(self, path, *, strength=1.0):
        ...
        """
        if self._operation_depth > 1:
            raise RuntimeError("Cannot change LoRA during generation")
        if path is None:
            self._lora = None
            return None
    ...
    def set_lora_strength(self, strength):
        ...
        if getattr(self, "_lora", None) is None:
            raise ValueError("Load a LoRA before setting its strength")
        self._lora_strength = validate_strength(strength)
        return self.lora_info

    def unload_lora(self):
        """Select the unchanged base model for subsequent synthesis."""
        return self.load_lora(None)

    @property
    def lora_info(self):
        adapter = getattr(self, "_lora", None)
        return adapter.info(self._lora_strength) if adapter is not None else None
    ...
    def close(self):
        if self._operation_depth > 1:
            raise RuntimeError("Cannot close pipeline during generation")
        if self.backend == "vllm":
            from .fast import close_vllm
            close_vllm(self)
        self._model, self._vae = None, None
    ...
    def effective_config(self, request, abc_sampling=None, semantic_sampling=None):
        ...
        return {"generation": config, "overrides": overrides,
                ...
                **({"lora": self.lora_info} if self.lora_info is not None else {})}
