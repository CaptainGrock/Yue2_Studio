...
(async()=>{
  ...
  let entered=null,renameCalls=0;
  state.jobs=[{id:'one',title:'Old name',starred:true}];
  context.window={prompt:()=>entered};
  context.api=async(url,data)=>{renameCalls++;assert.equal(url,'/api/jobs/one/rename');return {title:data.title};};
  await context.renameSong('one');assert.equal(renameCalls,0);
  entered='Old name';await context.renameSong('one');assert.equal(renameCalls,0);
  entered=' ';await context.renameSong('one');assert.equal(renameCalls,0);
  entered='x'.repeat(181);await context.renameSong('one');assert.equal(renameCalls,0);
  entered='  New name 🎵  ';await context.renameSong('one');
  assert.equal(state.jobs[0].title,'New name 🎵');assert.equal(state.jobs[0].starred,true);
  context.api=()=>new Promise(r=>resolve=r);
  entered='Another name';const renamePending=context.renameSong('one');
  state.jobs=[{id:'one',title:'New name 🎵',starred:true}];
  resolve({title:'Another name'});await renamePending;
  assert.equal(state.jobs[0].title,'Another name');
  context.api=async()=>{throw Error('save failed');};
  entered='Not saved';await context.renameSong('one');assert.equal(state.jobs[0].title,'Another name');
  assert(html.includes('id="i-pencil"'));
  console.log('Library dates, filters, stars, and title editing tests passed.');
