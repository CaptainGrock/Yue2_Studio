...
def run(args):
    ...
    try:
        print('SheetSage2: loading transcription runtime and models...', flush=True)
        import torch
        ...
        for key in ('overlap_seconds', 'lookahead_seconds', 'export_logits', 'export_scores',
                    'export_embeddings', 'output_hidden_states'):
            value = getattr(args, key, None)
            if value is not None:
                options[key] = value
        print('SheetSage2: transcribing source audio...', flush=True)
...
def main():
    ...
    parser.add_argument("--overlap-seconds", type=float, help="Override window overlap; default follows preset")
    parser.add_argument("--lookahead-seconds", type=float, help="Override right-hand context; default follows preset")
    parser.add_argument("--export-logits", action="store_true", help="Save raw per-window decoder logits")
    parser.add_argument("--export-scores", action="store_true", help="Save grammar-constrained decoder scores")
    parser.add_argument("--export-embeddings", action="store_true", help="Save MERT audio embeddings")
    parser.add_argument("--output-hidden-states", action="store_true", help="Save all MERT hidden layers")
